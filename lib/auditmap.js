...
function GetFindingObjects(doc) {
    ...
    for (var i=0; i<vFindings.length; i++) {
        ...
        var NewNodeEntry = {
            id: Nodes.length,
            label: 'Finding:\n#' + vNumber,
            title: 'Find. #' + vNumber +': ' + xpath.select("/Audit/Cases/Case[@nr='" + vFindings[i].nodeValue + "']/@nm",doc)[0].nodeValue,
            shape: 'diamond',
...
function GetRecommendationObjects(doc) {
    ...
    for (var i=0; i<vRecommendations.length; i++) {
        ...
        if (xpath.select("boolean(/Audit/Cases/Case[@nr='" + vRecommendations[i].nodeValue + "']/@number)",doc)){
            vNumber = xpath.select("/Audit/Cases/Case[@nr='" + vRecommendations[i].nodeValue + "']/@number",doc)[0].nodeValue
        } else {
            vNumber = 0;
        };        
        ...
        var NewNodeEntry = {
            ...
            title: 'Rec. #' + vNumber +': ' + xpath.select("/Audit/Recommendations/Recommendation[@nr='" + vRecommendations[i].nodeValue + "']/@category",doc)[0].nodeValue,  
            shape: 'triangle',
            group: 3
        };
