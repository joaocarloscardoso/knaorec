...
function LoadAuditRecommendations(fileid, sellang) {
    Catalog = [];
    tempDescription = '';
    tempTimeline ='';
    tempStatus='';
    ...
    for (var i=0; i<vRecommendations.length; i++) {
        if (xpath.select("/Audit/Recommendations/Recommendation[@nr='" + vRecommendations[i].nodeValue + "']/quote[@type='description' and @l='" + sellang +"']/.",doc)[0].firstChild === null)
        {
            tempDescription= '';
        }else{
            tempDescription= xpath.select("/Audit/Recommendations/Recommendation[@nr='" + vRecommendations[i].nodeValue + "']/quote[@type='description' and @l='" + sellang +"']/.",doc)[0].firstChild.nodeValue;
        };
        if (xpath.select("/Audit/Recommendations/Recommendation[@nr='" + vRecommendations[i].nodeValue + "']/quote[@type='timeline' and @l='" + sellang +"']/.",doc)[0].firstChild === null)
        {
            tempTimeline= '';
        }else{
            tempTimeline= xpath.select("/Audit/Recommendations/Recommendation[@nr='" + vRecommendations[i].nodeValue + "']/quote[@type='timeline' and @l='" + sellang +"']/.",doc)[0].firstChild.nodeValue;
        };
...
function LoadAuditRecommendationsForAnalysis(fileid) {
    ...
    var List  = {
        ...
        Recommendations: LoadAuditRecommendations(fileid, credentials.WorkLang.toLowerCase()),
