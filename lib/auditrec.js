...
function LoadAuditRecommendations(fileid) {
    ...
    for (var i=0; i<vRecommendations.length; i++) {
        ...
        for (var jF=0; jF<vColFindings.length; jF++) {
            if (jF === 0) {
                tempFindings = vColFindings[jF].nodeValue;
            } else {
                tempFindings = tempFindings + '|' + vColFindings[jF].nodeValue;
            }
        };
...
function LoadAuditRecommendationsForAnalysis(fileid) {
    ...
    List.NumberOfRepRecommendations = (parseInt(List.NumberOfRecommendations) - parseInt(List.NumberOfNewRecommendations))
