...
function GetTableQuery(VectorFile) {
    ...
    for (i=0; i < TotalTokens; i++){
        ...
        if (data[i].Label == 'Topics'){
            CypherQuery = CypherQuery + "MATCH (n1:Topics {Definition: '" + data[i].Name + "'})-[r:COVER]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Country'){
            CypherQuery = CypherQuery + "MATCH (n1:Country {Name: '" + data[i].Name + "'})-[r:PRODUCED]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Function'){
            CypherQuery = CypherQuery + "MATCH (n1:Function {Definition: '" + data[i].Name + "'})-[r:PERFORM]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Goal'){
            CypherQuery = CypherQuery + "MATCH (n1:Goal {Definition: '" + data[i].Name + "'})-[r:ACHIEVE]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Impact'){
            CypherQuery = CypherQuery + "MATCH (n1:Impact {Definition: '" + data[i].Name + "'})-[r:DETERMINE]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Process'){
            CypherQuery = CypherQuery + "MATCH (n1:Process {Definition: '" + data[i].Name + "'})-[r:EVALUATE]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Requirement'){
            CypherQuery = CypherQuery + "MATCH (n1:Requirement {Definition: '" + data[i].Name + "'})-[r:DETERMINE]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'ActivityDomain'){
            CypherQuery = CypherQuery + "MATCH (n1:ActivityDomain {Definition: '" + data[i].Name + "'})-[r:ADDRESS]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        } else if (data[i].Label == 'Risk_Case'){
            CypherQuery = CypherQuery + "MATCH (n1:Risk_Case {Definition: '" + data[i].Name + "'})-[r:IDENTIFIED]-(a1:Audit) return n1, a1, id(a1) as id LIMIT 10";
        }
    }
    var NodeResults= graphdb.GetListRecommendations(CypherQuery);
