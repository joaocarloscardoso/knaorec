...
function LoadAuditRecommendationsForAnalysis(doc, auditid, sellang) {
    ...
    //audit reference section
    tempRef = auditid;
    var res = xpath.select("/portfolio/audits/Audit/About[@id='" + auditid + "']/title/tx[@l='" + sellang + "']/@name",doc);
    if (res.length==1 && res[0] != null){
        tempTitle = res[0].nodeValue;
    } else {
        res = xpath.select("/portfolio/audits/Audit/About[@id='" + auditid + "']/title/tx[@l='" + credentials.WorkLang + "']/@name",doc);
        if (res.length==1 && res[0] != null){
            tempTitle = res[0].nodeValue;
        } else {
            tempTitle = '';
...
function AddAuditToPortfolio(id, AuditFileAttach, userid) {
    ...
    return new Promise(function(resolve, reject){
        database.QueryDataByID(id, dbfields, sortfields).then(function(Result){
            ...
            if (xpath.select("boolean(/portfolio/audits/Audit/About[@id='" + NewRefId[0].nodeValue + "']/title/tx[@l='" + credentials.WorkLang + "']/@name)",doc)){
                var vError = [  
    ...
    return new Promise(function(resolve, reject){
        database.QueryDataByID(portfolioid, dbfields, sortfields).then(function(Result){
            ...
            for (var i=0; i<vAudits.length; i++) {
                ...
                var res = xpath.select("/portfolio/audits/Audit/About[@id='" + vAudits[i].nodeValue + "']/title/tx[@l='" + sellang + "']/@name",doc);
                if (res.length==1 && res[0] != null){
