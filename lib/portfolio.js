...
function LoadPortfolioAuditsXML(fileid) {
    ...
    for (var i=0; i<vAudits.length; i++) {
        ...
        var NewEntry = {
            ...
            //,Cases:LoadPortfolioFindingsXML(fileid, vAudits[i].nodeValue),
            //Recommendations: LoadPortfolioRecommendationsXML(fileid, vAudits[i].nodeValue)
        };
...
function LoadPortfolioFindingsXML(fileid, auditid) {
    Catalog = [];
    tempDescription = '';
    vNumber=0;
    tempName = '';
...
function LoadPortfolioDataXML(fileid) {
    var data = fs.readFileSync(fileid, { encoding : 'UTF-8' });
    // Create an XMLDom Element:
    var doc = new Dom().parseFromString(data);
    ...
    var Porfolio = {
        ...
        org: xpath.select("/portfolio/@org",doc)[0].nodeValue,
        publish: xpath.select("/portfolio/@publish",doc)[0].nodeValue,
        audits: LoadPortfolioAuditsXML(fileid),
        treemap:''
    };
...
//generate portfolio xml from json database

function SavePortfolioDataJSON(fileid) {
    var data = fs.readFileSync(fileid, { encoding : 'UTF-8' });
    
    var Porfolio = XMLConvert.xml2json(data, {compact: true, spaces: 4});
...
module.exports.LoadPortfolioDataXML = LoadPortfolioDataXML;
module.exports.SavePortfolioDataJSON = SavePortfolioDataJSON;
