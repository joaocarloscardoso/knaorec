...
function InsertData(data){
    return new Promise(function(resolve, reject){
        MongoClient.connect(urlDB, { useUnifiedTopology: true }, function(err, db) {
            ...
            dbo.collection(credentials.mongoDB.colportfolio).insertOne(data, function(err, result) {
                if (err) throw err;
                console.log("Document inserted");
                resolve(result);
                db.close();
...
function UpdateData(myquery, data){
    MongoClient.connect(urlDB, { useUnifiedTopology: true }, function(err, db) {
        if (err) throw err;                
        var dbo = db.db(credentials.mongoDB.dbportfolio);        
        dbo.collection("notifications").updateOne(myquery, data, function(err, res) {
            if (err) throw err;
