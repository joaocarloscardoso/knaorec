...
function LoadRecommendationMatrix(fileid, RecommendationId) {
    ...
    if (RecommendationId != 'New') {
        ...
        var NewEntry = {
            ...
            Risks: 'Governance|Operational|Reputational|Financial|Compliance|Misappropriation of assets|Obsolescence',
            RisksEvaluation: 'High|Medium|Low',
            Description: tempDescription,
            ...
            MonStatus:'Not initiated|Cancelled|Deferred|Identified|In progress|Implemented',
            RelFindings: [],
            Monitoring: []
        };
...
function SaveRecommendationMatrix(fileid, Catalog) {
    ...
    console.log(xpath.select("/Audit/Recommendations",doc).length);
    ...
    if (Catalog.RecommendationId == '(New)'){
        ...
        var res = xpath.select("/Audit/Recommendations/Recommendation[@nr='" + RecommendationIdRef + "']/findings",doc);
        for (var x=0; x<Catalog.RelFindings.length; x++) {
            res[0].appendChild(doc.createElement('finding'));
            res[0].lastChild.setAttribute('nr', Catalog.RelFindings[x].RowId);
        }
        var res = xpath.select("/Audit/Recommendations/Recommendation[@nr='" + RecommendationIdRef + "']/monitoring",doc);
        for (var x=0; x<Catalog.Monitoring.length; x++) {
            res[0].appendChild(doc.createElement('assessment'));
            res[0].lastChild.setAttribute('nr', Catalog.Monitoring[x].RowId);
            res[0].lastChild.setAttribute('date', Catalog.Monitoring[x].Date);
            res[0].lastChild.setAttribute('status', Catalog.Monitoring[x].Status);
            res[0].lastChild.setAttribute('note', Catalog.Monitoring[x].Note);
        }
    } else {
        ...
        vItem2Delete = xpath.select("/Audit/Recommendations/Recommendation[@nr='" + Catalog.RecommendationId + "']/monitoring",doc);
        res[0].removeChild(vItem2Delete[0]);
        //then recreate them
        res[0].appendChild(doc.createElement('findings'));
        res[0].appendChild(doc.createElement('monitoring'));

        var res = xpath.select("/Audit/Recommendations/Recommendation[@nr='" + Catalog.RecommendationId + "']/findings",doc);
        for (var x=0; x<Catalog.RelFindings.length; x++) {
            res[0].appendChild(doc.createElement('finding'));
            res[0].lastChild.setAttribute('nr', Catalog.RelFindings[x].RowId);
        }
        var res = xpath.select("/Audit/Recommendations/Recommendation[@nr='" + Catalog.RecommendationId + "']/monitoring",doc);
        for (var x=0; x<Catalog.Monitoring.length; x++) {
            res[0].appendChild(doc.createElement('assessment'));
            res[0].lastChild.setAttribute('nr', Catalog.Monitoring[x].RowId);
            res[0].lastChild.setAttribute('date', Catalog.Monitoring[x].Date);
